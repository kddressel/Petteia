...
namespace Assets.Petteia.Scripts.Model
{
  ...
  public class DiceRollNumSpacesRulesSet : RulesSet
  {
    ...
    public DiceRollNumSpacesRulesSet(int maxSpaces, int numSpacesAllowedThisTurn) : base()
    {
      _maxSpaces = maxSpaces;
      if(numSpacesAllowedThisTurn >= 1)
      {
        _numSpacesAllowedThisTurn = numSpacesAllowedThisTurn;
      }
      else
      {
        Reroll();
    ...
    {
      Reroll();
      return base.AdvanceToNextTurn(players, currentTurn);
    }
  }
  ...
  public class RulesSet
  {
    ...
    virtual protected bool IsUnobstructed(BoardStateModel board, Vector2Int from, Vector2Int to)
    {
